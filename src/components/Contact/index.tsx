...
export default function Contact() {
  ...
  const whatsappMessage = encodeURIComponent(
    t('contact.whatsapp_message', { defaultValue: 'Hello Bruno, I am ready to stop losing sales. I would like to schedule a strategic diagnosis for my company.' })
  );
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${whatsappMessage}`;

  return (
    <section id="contact" className="relative py-24 bg-zinc-900 px-6 md:px-20 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center flex flex-col items-center">
        <h2 className="text-amber-500 font-bold tracking-widest uppercase text-sm mb-4">
          {t('contact.subtitle', { defaultValue: 'The next step' })}
        </h2>

        <h3 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white leading-tight mb-6">
          {t('contact.title_start', { defaultValue: 'Ready to stop losing sales and ' })}
          <span className="text-transparent bg-clip-text bg-linear-to-r from-amber-400 to-amber-600">
            {t('contact.title_highlight', { defaultValue: 'start scaling?' })}
          </span>
        </h3>

        <p className="text-zinc-400 text-lg md:text-xl mb-12 max-w-3xl leading-relaxed">
          {t('contact.description', { defaultValue: "As I work in-depth on projects focused on real results, my schedule availability is restricted. Let's schedule a strategic diagnosis of your company. Click below and contact me on WhatsApp." })}
        </p>
        ...
        >
          ...
          {t('contact.button_text', { defaultValue: 'Schedule Strategic Diagnosis' })}
        </a>
        ...
        <div className="mt-8 flex items-center justify-center space-x-2 text-zinc-500 text-sm">
          ...
          <span>{t('contact.security_info', { defaultValue: 'Your information is safe. Contact is directly with me.' })}</span>
        </div>
      </div>
    </section>
  );
